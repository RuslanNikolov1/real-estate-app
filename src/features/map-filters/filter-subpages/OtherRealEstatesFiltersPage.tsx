...
interface OtherRealEstatesFiltersPageProps {
    locationState?: {
        searchTerm: string;
        city: string;
    ...
    onFiltersChange?: (filters: OtherRealEstatesFiltersState) => void;
...
export function OtherRealEstatesFiltersPage({ 
    ...
    const notifyFiltersChange = useCallback(() => {
        if (onFiltersChange) {
            onFiltersChange(filterValuesRef.current as OtherRealEstatesFiltersState);
        }
    }, [onFiltersChange]);
    ...
    return (
        <div key={filterKey} className={styles.leftFiltersWrapper}>
            ...
            <PriceFilter
                ...
                priceSliderMax={OTHER_REAL_ESTATES_PRICE_SLIDER_MAX}
